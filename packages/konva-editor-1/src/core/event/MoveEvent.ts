...
function getPipelineNodes(node: Konva.Node) {
...
function getPipelineBindNodes(line: Konva.Line) {
...
export const bindMoveEvent = (layer: Konva.Layer) => {
  ...
  layer.on("dragstart.updatePipelines", (e) => {
    const node = getCurrentComponent(e.target);
    if (!node) return;
    const pipelineNodes = getPipelineNodes(node);
  ...
  layer.on("dragmove.updatePipelines", (e) => {
    ...
    pipelineNodes.forEach((itemLine) => {
      ...
      const { endNode: endNodeId, startNode: startNodeId } =
        getPipelineBindNodes(itemLine); // 需要再次获取绑定信息
