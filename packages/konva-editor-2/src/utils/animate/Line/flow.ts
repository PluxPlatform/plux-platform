...
export const flowLineAnimate = (
  ...
  const animLine = new Konva.Line({
    points: [],
    stroke: animLineColor,
    strokeWidth: animLineWidthRatio * baseStrokeWidth,
    lineCap: "round",
    lineJoin: "round",
  });
  ...
  let segmentIndex = 0;
  let progress = 0;
  let currentPoints: number[] = [points[0], points[1]]; // 初始化只有起点
  ...
  function growLine() {
    ...
    if (progress >= length) {
      // 画到这个点了
      currentPoints.push(x2, y2);
      segmentIndex++;

      if (segmentIndex >= points.length / 2 - 1) {
        // 整个路径画完了，重置
        setTimeout(() => {
          animLine.points([points[0], points[1]]);
          segmentIndex = 0;
          progress = 0;
          currentPoints = [points[0], points[1]];
          layer!.batchDraw();
    ...
    animLine.points([...currentPoints, currentX, currentY]);
